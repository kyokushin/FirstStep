...
class MusicPlayListDB {
	...
	{
		$this->db_link = mysql_connect('localhost', 'kenta', 'kenta');
	...
	public function getMusicUrl( $song_name, $artist_name)
	{
	  $query = "select url from music where song_name = \"$song_name\" AND artist_name = \"$artist_name\"";
	...
	public function getMusicId( $song_name, $artist_name)
	{
	  $query = "select id from music where song_name = \"$song_name\" AND artist_name = \"$artist_name\"";
