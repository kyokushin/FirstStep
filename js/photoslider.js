...
FOTO.Slider = {
	...
	imageCache : {}, // cache of the images so the preloaded images aren't
						// overwritten in some browsers
	loadingURL : '/articles/assets/slider/spinner.gif',
	baseURL : '/articles/assets/slider/',
	thumbURL : 't_{ID}.jpg',
	mainURL : '{ID}.jpg',
	thumbWidth : 100,// �ｽT�ｽ�ｽ�ｽl�ｽC�ｽ�ｽ�ｽT�ｽC�ｽY�ｽﾏ更�ｽﾇ会ｿｽ�ｽ�ｽ
	duration : 3000, // how long do we look at each image?

	data : {}, // contains things like current offset, paused, etc keyed off of
				// the slider id
	...
	importBucketFromIds : function(key, ids) {
		this.bucket[key] = new Object(); // replace any existing entries with
											// this new set
		for (i in ids) {
			this.bucket[key][ids[i]] = new Object(); // there isn't any
														// reason to pre-store
														// the urls for this
	...
	buildThumbBar : function(key, bar) {
		...
		for (i in this.bucket[key]) {
			...
			/*
			 * this.data[key]['thumbWidth'] = parseInt(div.css('width'))
		...
		// finally we need to force the width of the bar so that the divs don't
	...
	reload : function(key) {
		...
		// this.data[key]['thumbWidth'] = 72;�ｽT�ｽ�ｽ�ｽl�ｽC�ｽ�ｽ�ｽT�ｽC�ｽY�ｽﾏ更�ｽ�懶ｿｽ�ｽ
		this.data[key]['thumbWidth'] = this.thumbWidth;// �ｽ�ｽ�ｽﾇ会ｿｽ�ｽ�ｽ
		...
		// var firstThumb = $('#'+key+'
		// .photoslider_thumb[slot=0]');�ｽT�ｽ�ｽ�ｽl�ｽC�ｽ�ｽ�ｽT�ｽC�ｽY�ｽﾏ更�ｽ�懶ｿｽ�ｽ
		var firstThumb = $('#' + key + ' .photoslider_thumb[slot=0]');// �ｽ�ｽ�ｽﾇ会ｿｽ�ｽ�ｽ
		firstThumb.click();
	},
	...
	thumbClick : function(ev) {
		...
		// window.alert('id '+id+' bar '+bar+' nav '+nav+' parent '+parent+' key
		// '+key+' slot '+slot);
		...
		if (this.data[key]['intervalThumb'])
			SKEL.EFFECTS.Slide
					.stopByIntervalId(this.data[key]['intervalThumb']);
		...
		if (this.data[key]['currentId'] != null) {
			...
			SKEL.EFFECTS.Slide.animate($('.photoslider_thumb[imageid='
					+ this.data[key]['currentId'] + ']'), 'top', '0px', '0px',
					500, SKEL.Transitions.quadOut);
			// NOTE: we don't want to clear this interval incase the user clicks
			// really fast (unless we kept track of every animation)
		}
		...
		// animate our div down //下がらないようにへんこう
		this.data[key]['intervalThumb'] = SKEL.EFFECTS.Slide.animate(thumb,
				'top', '0px', '0px', 250, SKEL.Transitions.quadOut);
		...
		// alternative method
		// var centerStart = (-this.data[key]['thumbWidth'] * slot) +
		// ((this.data[key]['thumbWidth'] * (slots/2)) -
		// (this.data[key]['thumbWidth']/2));

		var centerStart = (navWidth / 2)
				- ((+this.data[key]['thumbWidth'] * (slot)) + (this.data[key]['thumbWidth'] / 2));
		// var centerStart = -1 * (this.data[key]['thumbWidth'] * slot); //use
		// this if you want it to be aligned on the left

		var barFrom = currentPos['x'] + 'px';
		var barTo = centerStart + 'px';

		// window.alert('thumb '+thumb+' key'+key+' from '+barFrom+' to '+barTo+
		// ' navWidth '+navWidth+' thumb '+this.data[key]['thumbWidth']+' slot
		// '+slot+' center'+centerStart);

		this.data[key]['interval'] = SKEL.EFFECTS.Slide.animate(bar, 'left',
				barFrom, barTo, 1000, SKEL.Transitions.backOut);
	},
	...
	setMainImage : function(key, id) {
		...
		this.preload(this.getUrlFromId(key, id, false),
				FOTO.Slider.displayMainImage, {
					key : key,
					id : id
				});
	},
	...
	preloadImages : function(key) {
		...
		if (this.bucket[key] != null && this.bucket[key][1] != null) {
			this.preload(this.getUrlFromId(key, 1, false),
					FOTO.Slider.preloadImageChain, {
						key : key,
						slot : 1
					});
		}
	...
	preloadImageChain : function() {
		...
		if (FOTO.Slider.bucket[key] != null
				&& FOTO.Slider.bucket[key][slot] != null) {
			FOTO.Slider.preload(FOTO.Slider.getUrlFromId(key, slot, false),
					FOTO.Slider.preloadImageChain, {
						key : key,
						slot : slot
					});
	...
	// returns the absolute coords based on where it is on the page (works w/
	// scrolled content)
	...
	// this is just like the findAbsolutePos, but stops if a parent's css is
	// "position:relative;"
	...
	cycleImage : function(key) {
		var slot = parseInt(parseInt(this.data[key]['currentSlot']) + 1); // force
																			// this
																			// to
																			// be a
																			// number
		...
		var thumb = $('#' + key + ' .photoslider_thumb[slot=' + slot + ']')
				.get(0);
		if (thumb == null) {
	...
	play : function(key) {
		if (this.data[key]['paused']) {
			this.data[key]['intervalCycle'] = setInterval(function() {
				FOTO.Slider.cycleImage(key);
			}, FOTO.Slider.duration);
		}
...
SKEL.Transitions = {
	 ...
	 * http://www.robertpenner.com/easing/ * http://mootools.net * * For more
	 * methods check * http://opiefoto.com/js/skel/transitions.js
	 */
...
SKEL.EFFECTS.Slide = {
	...
	animate : function(element, cssAttribute, from, to, duration, transition) {
		...
		// IE doesn't support arguments, so make a function that explicitly
		// calls with those arguments
		element.skel_animate_id = setInterval(function() {
			SKEL.EFFECTS.Slide.step(element, cssAttribute, from, to, duration,
	...
	step : function(element, cssAttribute, from, to, duration, start,
			transition) {
		...
		var newValue = SKEL.EFFECTS.Slide.compute(curTime, from, to, duration,
				start, transition);
		...
		// window.alert('css: '+cssAttribute+' new:'+newValue+' element:
		// '+element);
		...
		if (time > (startTime + duration)) {
			...
			if (typeof (from) == 'object') {
				...
				from.forEach(function(value, index) {
					newFrom = value;
					newTo = to[index];
					newValue = transitionFunc(deltaTime, newFrom,
	...
	// returns prefix,postfix,value eg: #ff0000 {prefix: #, value: ff0000},
	// abc123efg {prefix: abc, value: 123, postfix: efg}
